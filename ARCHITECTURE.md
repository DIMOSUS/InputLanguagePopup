...
| `InputLanguageService` | Resolves the layout-owning window → thread id → `GetKeyboardLayout`; converts the HKL to a display code (`RU`/`EN`/`LT`/ISO). The layout is per *thread*, and in hosted content (WebView2/Teams, UWP frames, Steam popups) the focused window belongs to a different thread — often another process — than the top-level window, and only that thread tracks the layout; so the `GUITHREADINFO` focus window is preferred, with consoles using `ImmGetDefaultIMEWnd` instead. Composes the popup text, appending `CAPS` when CapsLock is on. |
